...
class Distilling_Single_FRS_Diff(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 distill=None,):
        super(Distilling_Single_FRS_Diff, self).__init__(backbone, neck, bbox_head, train_cfg,
                                  test_cfg, pretrained)
        from mmdet.apis.inference import init_detector
    ...
    def forward_train(self,
        ...
        for layer in range(layers):
            ...
            stu_mask = torch.max(stu_cls_score_sigmoid, dim=1).values  # [n, h, w]
            stu_mask = stu_mask.detach()
            tea_mask = torch.max(tea_cls_score_sigmoid, dim=1).values  # [n, h, w]
            tea_mask = tea_mask.detach()
            # diff_mask
            diff_mask = (tea_mask - stu_mask).sigmoid()
            mask = diff_mask.detach()
