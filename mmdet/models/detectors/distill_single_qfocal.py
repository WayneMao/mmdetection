...
class Distilling_Single_QFocal(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 distill=None,):
        super(Distilling_Single_QFocal, self).__init__(backbone, neck, bbox_head, train_cfg,
                                  test_cfg, pretrained)
        from mmdet.apis.inference import init_detector
    ...
    def forward_train(self,
        ...
        for layer in range(layers):
            ...
            # todo QFocal loss  beta=2, .pow(beta)
            cls_loss = (stu_cls_score_sigmoid - tea_cls_score_sigmoid).pow(2) * F.binary_cross_entropy(stu_cls_score_sigmoid, tea_cls_score_sigmoid,reduction='none')
            # todo BCE 连续域拓展

            distill_feat_loss += feat_loss.mean()
            distill_cls_loss +=  cls_loss.sum() / tea_cls_score_sigmoid.sum()
