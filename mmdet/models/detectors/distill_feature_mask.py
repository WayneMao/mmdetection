...
class Distilling_Feature_Mask(SingleStageDetector):
    '''
    feature level层， 学习[n,1,h,w] 单层layer作为mask;
    logit 还是沿用原来的FRS,
    '''
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 distill=None,):
        super(Distilling_Feature_Mask, self).__init__(backbone, neck, bbox_head, train_cfg,
                                  test_cfg, pretrained)
        from mmdet.apis.inference import init_detector
        ...
        num_layers = 5
        fpn_channel = 256  # 9*80
        mid_channel = fpn_channel // 2
        for _ in range(num_layers):
            self.mask_conv.append(
                nn.Sequential(
                nn.Conv2d(fpn_channel, mid_channel, 3, padding=(1, 1)),
                nn.BatchNorm2d(mid_channel),
                nn.ReLU(),
                nn.Conv2d(mid_channel, 1, 3, padding=(1, 1)),
                nn.BatchNorm2d(1),
            ))
    ...
    def forward_train(self,
        ...
        for layer in range(layers):
            ...
            # ---------------- conv mask ------------------------ #
            feature_mask = self.mask_conv[layer](x[layer])
            mask = feature_mask.sigmoid()  # [n,1,h,w]
